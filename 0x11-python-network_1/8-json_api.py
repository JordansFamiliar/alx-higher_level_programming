...
command-line argument, sends a POST request to
http://0.0.0.0:5000/search_user with the letter
as a parameter, and processes the response.
It attempts to parse the response as JSON and
extracts 'id' and 'name' fields if available.
It then prints the result if found, or "No result"
if no valid data is present in the response.
"""
